...
const DynamicPro = async (option: DynamicProOption) => {
  if (!option.name) option.name = "Not Found";
  if (!option.author) option.author = "NyuBot";
  ...
  if (!option.duration) option.duration = "Not Found";
  if (!option.requested) option.requested = "Nyu";
